...
% Networks).
%JEEO It has extended to design a Faster -RCNN Model able to detect motorcycles in 10K examples 
%
...
%% Load Dataset
% This example is extrapolated to use the 10000 examples of motrocycles
% practice, more labeled images are needed to train a robust detector.
clear all
close all
clc
% Load vehicle data set
data = load('Motos10k.mat');
...
dataDir = fullfile('D:\DeepLearning\DeepLearningEnvironment\DeepLearning\matLab\icprs2018\images10k');
vehicleDataset.imageFilename = fullfile(dataDir, vehicleDataset.imageFilename);


% Read one of the images.
I = imread(vehicleDataset.imageFilename{8000});

% Insert the ROI labels.
I = insertShape(I, 'Rectangle', vehicleDataset.motorbike{8000});
...
%JEEO for work with only 300 examples
%vehicleDataset=vehicleDataset(1:200,:);
...
% JEEO - Extended to Deep Learning since we have 10K examples using 90% of training without risk of overfitting
idx = floor(0.9 * height(vehicleDataset));
trainingData = vehicleDataset(1:idx,:);
testData = vehicleDataset(idx:end,:);
%testData=vehicleDataset;
...
%JEEO -Extended from the original version
filterSize = [3 3];
numFilters1 = 64;
numFilters2 = 32;

% Create the middle layers.
% JEEO- This configuration obey to necesity to detect occluded motrobukes in the scene.
middleLayers = [
                
    convolution2dLayer(filterSize, numFilters1, 'Padding', 1)   
    reluLayer()
    convolution2dLayer(filterSize, numFilters2, 'Padding', 1)  
    reluLayer() 
...
I = imread(vehicleDataset.imageFilename{150});

% Run the detector.
[bboxes, scores,labels] = detect(detector, I);

% Annotate detections in the image.
if  countcats(labels)~=0
...
%% JEEO - This sections is used to write a Video result of the detection
% clear all
% close all
% load('moto7500-DeepModel64_32_64_128.mat');
% load('MotosAll.mat');
% cd 'D:\DeepLearning\allDron'
% Create video player object
hsnk = vision.VideoPlayer('Position',[100 100 660 400]);
% Record the Video
writerObj = VideoWriter('D:\DeepLearning\DeepLearningEnvironment\DeepLearning\Videos\Results\EspiNet-10k-.avi');
open(writerObj);

for i = 1:height(vehicleDatasetAll)
        
        % Read the image.
        I = imread(vehicleDatasetAll.imageFilename{i});
...
% proposals from edges." _European Conference on Computer Vision_ 2014.
